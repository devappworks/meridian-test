        ...
        <div class="sidebar-news">
          ...
          <div v-else class="related-news-list">
            ...
            >
              ...
              <div class="content">
                <div class="category volleyball">ODBOJKA</div>
                <h3>{{ news.title }}</h3>
                <div class="timestamp">
                  <span>{{ news.date }}</span>
...
const VOLLEYBALL_CATEGORY_ID = 37;
...
export default {
  ...
  data() {
    return {
      ...
      volleyballNews: [],
      loadMoreVolleyballNews: [],
      otherNews: [],
      relatedNews: [],
      currentPage: 1,
      isLoading: false,
      hasMorePages: true,
      currentCategory: VOLLEYBALL_CATEGORY_ID,
      mainVolleyballCategory: VOLLEYBALL_CATEGORY_ID,
      volleyballMenuData: null,
      currentCategoryName: "ODBOJKA",
      categoryMapping: {},
      loading: {
        featured: true,
        main: true,
  ...
  computed: {
    pageTitle() {
      if (this.volleyballMenuData && this.currentCategoryName !== "ODBOJKA") {
        return `ODBOJKA - ${this.currentCategoryName}`;
      }
      return "ODBOJKA";
    },
  },
  methods: {
    ...
    buildCategoryArray() {
      if (this.currentCategory === this.mainVolleyballCategory) {
        return this.mainVolleyballCategory;
      } else {
    ...
    async fetchWebSettings() {
      try {
        ...
        if (webSettings.success && webSettings.result.languages?.length > 0) {
          ...
          const volleyballMenu = defaultLanguage.web_menu?.find(
            (menu) => menu.title === "ODBOJKA"
          );

          if (volleyballMenu) {
            // Store the entire volleyball menu data for navigation
            this.volleyballMenuData = volleyballMenu;
            ...
            if (volleyballMenu.web_categories?.length > 0) {
              ...
              this.mainVolleyballCategory = mainCategoryId;
            ...
            if (volleyballMenu.sub_menu?.length > 0) {
              ...
              this.categoryMapping[this.currentCategory] = "ODBOJKA";
            }
    ...
    async switchCategory(categoryId) {
      ...
      this.currentCategoryName = this.categoryMapping[categoryId] || "ODBOJKA";

      this.isSwitchingCategory = true;

      try {
        await this.fetchVolleyballArticles();
      } finally {
      ...
      window.dispatchEvent(
        new CustomEvent("volleyball-category-updated", {
          detail: { categoryId },
        })
      );
    },
    ...
    async fetchVolleyballArticles() {
      if (this.isSwitchingCategory) {
        ...
        this.loading = {
          ...
          main: this.volleyballNews.length === 0,
          loadMore: false,
      ...
      try {
        const volleyballData = await fetchFromApi("/getArticles", {
          articleLimit: 50,
          "category[]": this.buildCategoryArray(),
        });

        const allArticles = volleyballData.result.articles;
        ...
        if (filteredArticles.length > 0) {
          ...
          this.loadMoreVolleyballNews = filteredArticles
            .slice(17, 29)
            .map(this.mapArticle);
        ...
        console.error("Error fetching volleyball articles:", error);
        this.resetNews();
        this.loading = {
          featured: false,
          main: false,
        ...
        while (collectedArticles.length < targetCount && this.hasMorePages) {
          ...
          if (filteredNewArticles.length > 0) {
            const existingUrls = new Set([
              ...this.volleyballNews.map((article) => article.url),
              ...this.loadMoreVolleyballNews.map((article) => article.url),
  ...
  async mounted() {
    ...
    this.fetchVolleyballArticles();

    window.addEventListener(
      "volleyball-category-changed",
