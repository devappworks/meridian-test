        ...
        <div class="sidebar-news">
          ...
          <div v-else class="related-news-list">
            ...
            >
              ...
              <div class="content">
                <div class="category football">FUDBAL</div>
                <h3>{{ news.title }}</h3>
                <div class="timestamp">
                  <span>{{ news.date }}</span>
...
export default {
  ...
  methods: {
    ...
    buildCategoryArray() {
      if (this.currentCategory === this.mainFootballCategory) {
        return this.mainFootballCategory;
      } else {
    ...
    async fetchWebSettings() {
      try {
        ...
        if (webSettings.success && webSettings.result.languages?.length > 0) {
          ...
          const footballMenu = defaultLanguage.web_menu?.find(
            (menu) => menu.title === "FUDBAL"
          );

          if (footballMenu) {
            // Store the entire football menu data for navigation
            this.footballMenuData = footballMenu;
            ...
            if (footballMenu.web_categories?.length > 0) {
              ...
              this.mainFootballCategory = mainCategoryId;
            ...
            this.currentCategoryName =
              this.categoryMapping[this.currentCategory] || "FUDBAL";
          }
        }
      } catch (error) {
    ...
    async switchCategory(categoryId) {
      ...
      this.currentCategoryName = this.categoryMapping[categoryId] || "FUDBAL";

      this.isSwitchingCategory = true;

      try {
        await this.fetchFootballArticles();
      } finally {
      ...
      window.dispatchEvent(
        new CustomEvent("football-category-updated", {
          detail: { categoryId },
        })
      );
    },
    ...
    async fetchFootballArticles() {
      if (this.isSwitchingCategory) {
        ...
        this.loading = {
          ...
          main: this.footballNews.length === 0,
          loadMore: false,
      ...
      try {
        const footballData = await fetchFromApi("/getArticles", {
          articleLimit: 50,
          "category[]": this.buildCategoryArray(),
        });

        const allArticles = footballData.result.articles;
        ...
        if (filteredArticles.length > 0) {
          ...
          this.footballNews = filteredArticles
            .slice(1, 17)
            .map(this.mapArticle);
          this.loadMoreFootballNews = filteredArticles
            .slice(17, 29)
            .map(this.mapArticle);
        ...
        while (collectedArticles.length < targetCount && this.hasMorePages) {
          ...
          if (filteredNewArticles.length > 0) {
            const existingUrls = new Set([
              ...this.footballNews.map((article) => article.url),
              ...this.loadMoreFootballNews.map((article) => article.url),
  ...
  data() {
    return {
      ...
      hasMorePages: true,
      currentCategory: 28,
      mainFootballCategory: 28,
      footballMenuData: null,
      currentCategoryName: "FUDBAL",
      categoryMapping: {},
      loading: {
        featured: true,
        main: true,
  ...
  async mounted() {
    ...
    this.fetchFootballArticles();

    window.addEventListener(
      "football-category-changed",
