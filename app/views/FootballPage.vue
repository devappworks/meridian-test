        ...
        <div class="sidebar-news">
          ...
          <div v-else class="related-news-list">
            ...
            >
              ...
              <div class="content">
                <div class="category football">{{ news.sport }}</div>
                <h3>{{ news.title }}</h3>
                <div class="timestamp">
                  <span>{{ news.date }}</span>
...
const { data: footballData, pending: footballPending } = await useAsyncData('football-articles', () => 
...
const switchCategory = async (categoryId) => {
  ...
  try {
    const footballDataResponse = await fetchFromApi("/getArticles", {
      articleLimit: 50,
